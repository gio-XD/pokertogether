...
export function ActionBar({ gameState, playerId }: ActionBarProps) {
  ...
  if (!isMyTurn || !player) {
    return (
      <div className="h-20 bg-[var(--panel-bg)] border-t border-[var(--panel-border)] flex items-center justify-center">
  ...
  return (
    <div className="bg-[var(--panel-bg)] border-t border-[var(--panel-border)] px-4 py-3">
      {/* Raise slider */}
      {showRaise && canRaise && (
        <RaiseSlider
      ...
      <div className="flex items-center justify-center gap-2 sm:gap-3 flex-wrap">
        ...
        {canCheck ? (
          <button
            onClick={handleCheck}
            className="px-6 py-3 rounded-xl font-bold text-sm text-white transition-all
              hover:brightness-110 active:scale-95 min-w-[100px]"
